...
class IHMetricCard(IHCard):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(master, padding=12, variant=variant, **kwargs)
        ...
        if helper:
            self._label(f"  ·  {helper}", role="muted", parent=header).pack(side="left", anchor="w")
        if badge:
            self._label(badge, role="muted", parent=header).pack(side="right", anchor="e")
        self._label(value, role="title").pack(anchor="w", pady=(4, 0))
        if delta:
            ...
            self._label(delta_text, role="muted").pack(anchor="w", pady=(3, 0))
